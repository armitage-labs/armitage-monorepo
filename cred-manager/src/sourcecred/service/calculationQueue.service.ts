...
export class CalculationQueueService {
  ...
  async handleCron() {
    ...
    if (semaphore.available) {
      const unhandledCalculation =
        await this.prismaService.contributionRequest.findFirst({
      ...
      if (unhandledCalculation) {
        ...
        try {
          ...
          await this.sourceCredService.calculateCredScores(
            unhandledCalculation.team_id,
            unhandledCalculation.access_token,
            unhandledCalculation.email,
          );
        } catch (e) {
          this.logger.error(e);
        }
